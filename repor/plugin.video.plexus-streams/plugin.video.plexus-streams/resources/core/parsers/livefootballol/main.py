...
base_url = "https://www.livefootballol.me"
...
def lfol_menu():
	addDir('Agenda','',401,os.path.join(current_dir,"icon.png"),1,True,parser="livefootballol",parserfunction="lfol_schedule")
	addDir('Canales','',401,os.path.join(current_dir,"icon.png"),1,True,parser="livefootballol",parserfunction="lfol_channels")
	
def lfol_channels():
	addDir('Canales Acestream','',401,os.path.join(current_dir,"icon.png"),1,True,parser="livefootballol",parserfunction="lfol_channels_ace")

def lfol_channels_ace():
	try:
		source = requests.get(base_url+'/acestream-channel-list-2018.html').text
...
def lfol_schedule():
	try:
		source = requests.get(base_url+'/live-football-streaming-2019.html').text
	except: source="";xbmcgui.Dialog().ok(translate(40000),translate(40128))
	if source:
