...
addon = Addon('plugin.video.lfp', sys.argv)
...
class main():
    ...
    def channels(self):
        result = client.request('http://bit.ly/2W6LCHH', headers=self.headers)
...
#           zx+=rw
#           zx+='\n\n---------------------\n\n'
#       f = open('C:/Users/Teco/AppData/Roaming/Kodi/addons/plugin.video.lfp/zRow.txt','w+')
...
#       f.close()
    ...
    def __prepare_events(self,events):
        ...
        for event in events:
          ...
          # f = open('C:/Users/Teco/AppData/Roaming/Kodi/addons/plugin.video.lfp/ztab.txt','w+')
          # f.write(str(len(src)))
          # f.close()
    ...
    def __prepare_links(self,links,tit):
        ...
        if new!=[]:
            ace.append(('x','[COLOR gold]LFP[/COLOR]',''))
        new = ace + new
        if spc!=[]:
            new.append(('x','[COLOR gold]LFP[/COLOR]',''))
        if spc!=[]:
            new = new + spc
        return new
